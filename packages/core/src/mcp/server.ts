...
export class OxygenMCPServer {
  private agents: Map<string, ConnectedAgent> = new Map();
  private mainAgents: Map<string, Set<string>> = new Map();
  private taskQueues: Map<string, MCPServerTaskQueue> = new Map();
  private waitingSubAgents: Map<string, string[]> = new Map();
  private taskCallbacks: Map<string, (task: MCPServerAgentTask) => void> = new Map();
  ...
  assignTask(config: {
    ...
    if (!subAgentId) {
      ...
      const availableSubAgents = subAgents.filter(sa => {
        ...
        return !queue.inProgress.some((t: MCPServerAgentTask) => t.subAgentId === sa.id);
      });
  ...
  waitForTask(
    subAgentId: string,
    callback: (task: MCPServerAgentTask) => void
  ): void {
    ...
    if (queue && queue.pending.length > 0) {
      const taskIndex = queue.pending.findIndex((t: MCPServerAgentTask) => t.subAgentId === subAgentId);
      if (taskIndex !== -1) {
   ...
   */
  private notifyWaitingSubAgent(subAgentId: string): void {
    const callback = this.taskCallbacks.get(subAgentId);
    const agent = this.agents.get(subAgentId);
    if (!callback || !agent || !agent.mainAgentId) return;
  ...
  deliverTask(config: {
  ...
  }): { success: boolean; task?: MCPServerAgentTask; error?: string } {
    ...
    const taskIndex = queue.inProgress.findIndex((t: MCPServerAgentTask) => t.id === config.taskId);
    if (taskIndex === -1) {
  ...
  getTask(mainAgentId: string, taskId: string): MCPServerAgentTask | undefined {
    ...
    return [
      ...
      ...queue.completed,
      ...queue.failed
    ].find((t: MCPServerAgentTask) => t.id === taskId);
  }
...
export function createMCPServer(): OxygenMCPServer {
