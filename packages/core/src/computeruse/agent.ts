...
import {
  ...
  GUIContext,
  ComputerUseConfig,
  COMPUTERUSE_SYSTEM_PROMPT,
  ScreenshotOutput,
} from './types';
import { createMockOperator } from './mock-operator';
import { parseGUIAction } from './action-parser';
...
export class ComputerUseAgent {
  ...
  private async runAgentLoop(task: ComputerUseTask): Promise<void> {
    ...
    while (this.stepCount < this.maxSteps && task.status === 'running' && !this.stopped) {
      ...
      try {
        ...
        const response = await callLLM(this.model, messages, {
          temperature: 0.2,
          maxTokens: 2048,
  ...
  private buildSystemPrompt(): string {
    let prompt = COMPUTERUSE_SYSTEM_PROMPT;
...
Please observe the screen and complete the task step by step. Start by taking stock of what's currently on screen, then plan your approach.`;
  ...
  private buildScreenMessage(goal: string, screenshot: ScreenshotOutput): any {
