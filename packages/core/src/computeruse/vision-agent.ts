...
import { createMockOperator } from './mock-operator';
import { parseGUIAction, formatGUIAction } from './action-parser';
import { ModelConfig } from '../types';
import { callLLM } from '../llm/provider';
import { ComputerUseTask, ComputerUseStep } from './agent';
...
export class VisionComputerUseAgent {
  private id: string;
  private model: ModelConfig;
  private visionModel: ModelConfig;
  private operator: GUIOperator;
  private config: ComputerUseConfig;
  private maxSteps: number;
  private initialContext: string;
  private coordinateSystem: 'top-left' | 'bottom-left' | 'center';
  private currentTask?: ComputerUseTask;
  private stepCount = 0;
  ...
  constructor(options: VisionAgentOptions) {
    ...
    this.operator = options.operator || createMockOperator();
    this.config = {
      operatorType: 'mock',
  ...
  async executeTask(goal: string): Promise<ComputerUseTask> {
  ...
  private async runAgentLoop(task: ComputerUseTask): Promise<void> {
    ...
    while (this.stepCount < this.maxSteps && task.status === 'running' && !this.stopped) {
      ...
      try {
        ...
        this.coordTransform = this.buildCoordinateTransform(
          visionAnalysis.imageWidth,
          visionAnalysis.imageHeight,
          screenshot.width,
          screenshot.height
        );
        ...
        const response = await callLLM(this.model, visionHistory as any, {
          temperature: 0.2,
          maxTokens: 2048,
        ...
        const action = parseGUIAction(assistantContent);
  ...
  private async analyzeScreenshot(screenshot: ScreenshotOutput): Promise<{
    summary: string;
    imageWidth: number;
    imageHeight: number;
    elements: Array<{ type: string; description: string; x: number; y: number }>;
  }> {
...
坐标格式请使用归一化的 [x, y]，范围 0-1，同时也给出像素坐标。`
    ...
    try {
      const response = await callLLM(this.visionModel, visionMessages as any, {
        temperature: 0.1,
        maxTokens: 1024,
        signal: this.abortController?.signal,
      });
  ...
  private convertActionCoordinates(action: GUIAction): GUIAction {
  ...
  private buildVisionUserMessage(
    ...
    visionAnalysis: { imageWidth: number; imageHeight: number; summary: string }
  ): any[] {
...
action_name(param='value')
  ...
  private buildUserPrompt(goal: string): string {
...
export function createVisionComputerUseAgent(
