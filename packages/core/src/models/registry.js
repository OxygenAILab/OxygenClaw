...
exports.ModelRegistry = exports.PROVIDER_COLORS = exports.MODEL_ICONS = exports.STANDARD_MODEL_NAMES = void 0;
exports.createModelRegistry = createModelRegistry;
exports.normalizeModelName = normalizeModelName;
exports.suggestModelNames = suggestModelNames;
exports.getModelIconFromKeywords = getModelIconFromKeywords;
const uuid_1 = require("uuid");
exports.STANDARD_MODEL_NAMES = {
...
class ModelRegistry {
    ...
    findModelByFuzzyMatch(query) {
        ...
        for (const model of this.models.values()) {
            ...
            if (model.provider.toLowerCase().includes(lowerQuery))
                score += 2;
            if (score > bestScore) {
                bestScore = score;
                bestMatch = model;
    ...
    getModelIconFromName(name) {
        const lowerName = name.toLowerCase();
        for (const [keyword, icon] of Object.entries(exports.MODEL_ICONS)) {
            if (lowerName.includes(keyword)) {
                return icon;
            }
        }
        return exports.MODEL_ICONS['default'];
    }
    ...
    getProviderColor(provider) {
        const lowerProvider = provider.toLowerCase();
        return exports.PROVIDER_COLORS[lowerProvider] || '#666666';
    }
...
function normalizeModelName(name) {
    ...
    for (const [standard, aliases] of Object.entries(exports.STANDARD_MODEL_NAMES)) {
...
function suggestModelNames(input, limit = 5) {
    const lower = input.toLowerCase();
    const suggestions = [];
    for (const standard of Object.keys(exports.STANDARD_MODEL_NAMES)) {
        if (standard.includes(lower)) {
            suggestions.push({ name: standard, score: 10 });
        }
        for (const alias of exports.STANDARD_MODEL_NAMES[standard]) {
...
function getModelIconFromKeywords(keywords) {
    for (const keyword of keywords) {
        const icon = exports.MODEL_ICONS[keyword.toLowerCase()];
        if (icon)
            return icon;
    }
    return exports.MODEL_ICONS['default'];
