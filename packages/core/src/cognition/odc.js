...
exports.ODCEngine = void 0;
exports.createODCEngine = createODCEngine;
const zod_1 = require("zod");
const types_1 = require("../types");
const REASONING_PROMPT = zod_1.z.object({
    analysis: zod_1.z.string(),
    confidence: zod_1.z.number(),
    nextAction: zod_1.z.enum(['continue', 'search', 'tool', 'delegate', 'output']),
...
class ODCEngine {
    ...
    async think(prompt, context, availableTools) {
        this.iterationCount = 0;
        this.reasoningHistory = [];
        this.taskDecomposition = [];
        const modeConfig = types_1.MODE_CONFIGS[this.mode];
        ...
        while (this.iterationCount < this.maxIterations) {
            this.iterationCount++;
            const result = await this.reasonAtLevel(currentLevel, prompt, context, availableTools, confidence);
    ...
    async reasonAtLevel(level, prompt, context, tools, previousConfidence) {
        switch (level) {
            case 'L1':
    ...
    async l1FastResponse(prompt, context, tools) {
    ...
    async l2StepByStep(prompt, context, tools) {
    ...
    async l3Reflective(prompt, context, tools, previousConfidence) {
    ...
    async l4MultiPath(prompt, context, tools) {
    ...
    async l5Collaborative(prompt, context, tools) {
    ...
    detectSearchNeed(prompt, keywords) {
    ...
    performSelfAssessment(prompt, context, tools, previousConfidence) {
        const issues = [];
        const recommendations = [];
        let adjustment = 0;
        let shouldStop = false;
        let nextAction = 'output';
    ...
    exploreMultiplePaths(prompt, tools) {
        const paths = [
            {
    ...
    getCoordinationStrategy(agents) {
        if (agents.length <= 2) {
...
exports.ODCEngine = ODCEngine;
function createODCEngine(mode, interactionMode = 'chat') {
    const config = types_1.MODE_CONFIGS[mode];
